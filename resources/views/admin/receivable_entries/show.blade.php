        ...
        <div class="wraper-content">
            <div class="row">
                <div class="col-sm-12 page-info">
                    <h2>{{trans('content.receivable_entry')}}</h2>
                    <div>
                        <a href="">{{trans('content.home')}}</a> -
                        <span>{{trans('content.receivable_entry')}}</span>
                    </div>
                </div>
                <div class="col-sm-12 table-data">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @isset($receivable_entry_details)
                                @foreach($receivable_entry_details as $_item)
                                    <tr>
                                        <td>{{$_item->Item->id}}</td>
                                        <td>{{$_item->Item->name}}</td>
                                        <td>{{$receivable_entry->Branch->name}}</td>
                                        <td>{{$receivable_entry->Store->name}}</td>
            ...
            $customiseData =
                "<div class='payable-custom-data'>\
                    <div class='payable-branch'>\
                        <span>{{trans('content.branch') . " / " . $receivable_entry->Branch->name}}</span>\
                    </div>\
                    <div class='payable-date'>\
                        <span>{{trans('content.vendorname') . " / " . $receivable_entry->Vendor->name}}</span>\
                    </div>\
                    <div class='payable-date'>\
                        <span>{{trans('content.date') . " / " . $receivable_entry->created_at}}</span>\
                    </div>\
                    <div class='payable-code'>\
                        <span>{{trans('content.receivablecode') . " / " . $receivable_entry->id}}</span>\
                    </div>\
                </div>";
            let language_ar = {
                ...
                buttons:  [
                    {
                        "charset": "utf-8",
                        "extend": "print",
                        "title": "{{trans('content.receivable_entry')}}",
                        "text": "{{trans('content.print')}}",
                        "filename": "{{trans('content.receivable_entry')}}",
                        autoPrint: true,
                        exportOptions: {
                    ...
                    },
                    {
                        ...
                        "extend": "csv",
                        "title": "{{trans('content.receivable_entry')}}",
                        "messageTop": "{{trans('content.date') . " / " . ($receivable_entry->created_at)}}",
