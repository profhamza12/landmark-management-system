            ...
            <div class="row">
                <div class="col-sm-12 page-info">
                    ...
                    <div>
                        ...
                        <span>{{trans('content.showOutStock')}}</span>
                    </div>
                </div>
                            ...
                            @isset($store_quantities)
                                @foreach($store_quantities as $_item)
                                    <tr>
                                        ...
                                        <td>{{$out_stock->Branch->name}}</td>
                                        <td>{{$out_stock->Store->name}}</td>
            ...
            $customiseData =
                "<div class='payable-custom-data'>\
                    <div class='payable-branch'>\
                        <span>{{trans('content.branch') . " / " . $out_stock->Branch->name}}</span>\
                    </div>\
                    <div class='payable-date'>\
                        <span>{{trans('content.date') . " / " . $out_stock->created_at}}</span>\
                    </div>\
                    <div class='payable-code'>\
                        <span>{{trans('content.payable_code') . " / " . $out_stock->id}}</span>\
                    </div>\
                </div>";
            let language_ar = {
            ...
            $('table').DataTable({
                ...
                buttons:  [
                    ...
                    },
                    {
                        ...
                        "extend": "csv",
                        "title": "{{trans('content.out_stock')}}",
                        "messageTop": "{{trans('content.date') . " / " . ($out_stock->created_at)}}",
