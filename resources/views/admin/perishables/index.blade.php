            ...
            <div class="row">
                <div class="col-sm-12 page-info">
                    ...
                    <div>
                        ...
                        <span>{{trans('content.perishables')}}</span>
                    </div>
                </div>
                <div class="col-sm-12 table-data">
                    ...
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                <th>{{trans('content.perishableId')}}</th>
                        ...
                        <tbody>
                            @isset($perishables)
                                @foreach($perishables as $perishable)
                                    <tr class="tr_{{$perishable->id}}">
                                        <td>{{$perishable->id}}</td>
                                        <td>{{$perishable->created_at}}</td>
                                        <td>
                                            <a href="{{route('perishables.show', $perishable->id)}}" class="btn btn-outline-info">{{trans('content.showPerishable')}}</a>
                                            @if($perishable->relayed == 0)
                                                <a href="{{route('perishables.edit', $perishable->id)}}" class="btn btn-outline-primary">{{trans('content.edit')}}</a>
                                                <a href="" data-id="{{$perishable->id}}" class="delete perishable-delete btn btn-outline-danger">{{trans('content.delete')}}</a>
                                                <a href="" data-id="{{$perishable->id}}" class="relay perishable-relay btn btn-outline-info">{{trans('content.relay')}}</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                ...
                buttons:  [
                    {
                        ...
                        "filename": "{{trans('content.perishables')}}",
                        autoPrint: true,
                        exportOptions: {
                            columns: [ 0, 1 ]
                        },
...
@section('ajaxscript')
    <script>
        $(function () {
            $(".perishable-delete").click(function (e) {
                e.preventDefault();
                ...
                if (window.confirm("{{trans('content.entrydeletepromptmsg')}}"))
                {
                    ...
                    let url = "{{route('perishables.destroy', ":id")}}";
            ...
            $(".perishable-relay").click(function (e) {
                e.preventDefault();
                $(".success, .ajax-success").hide();
                $(".error, .ajax-error").hide();
                ...
                {
                    let entryId = $(this).data('id');
                    let url = "{{route('perishables.relay', ":id")}}";
                    url = url.replace(":id", entryId);
                    $.ajax({
                        method: 'POST',
                        url: url,
