        ...
        <div class="wraper-content">
            <div class="row">
                <div class="col-sm-12 page-info">
                    <h2>{{trans('content.perishable')}}</h2>
                    <div>
                        <a href="">{{trans('content.home')}}</a> -
                        <span>{{trans('content.showPerishable')}}</span>
                    </div>
                </div>
                <div class="col-sm-12 table-data">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @isset($perishable_details)
                                @foreach($perishable_details as $_item)
                                    <tr>
                                        <td>{{$_item->Item->id}}</td>
                                        <td>{{$_item->Item->name}}</td>
                                        <td>{{$perishable->Branch->name}}</td>
                                        <td>{{$perishable->Store->name}}</td>
            ...
            $customiseData =
                "<div class='payable-custom-data'>\
                    <div class='payable-branch'>\
                        <span>{{trans('content.branch') . " / " . $perishable->Branch->name}}</span>\
                    </div>\
                    <div class='payable-date'>\
                        <span>{{trans('content.date') . " / " . $perishable->created_at}}</span>\
                    </div>\
                    <div class='payable-code'>\
                        <span>{{trans('content.perishableId') . " / " . $perishable->id}}</span>\
                    </div>\
                </div>";
            let language_ar = {
                ...
                buttons:  [
                    {
                        "charset": "utf-8",
                        "extend": "print",
                        "title": "{{trans('content.perishable')}}",
                        "text": "{{trans('content.print')}}",
                        "filename": "{{trans('content.perishable')}}",
                        autoPrint: true,
                        exportOptions: {
                    ...
                    },
                    {
                        ...
                        "extend": "csv",
                        "title": "{{trans('content.perishable')}}",
                        "messageTop": "{{trans('content.date') . " / " . ($perishable->created_at)}}",
