            ...
            <div class="row">
                <div class="col-sm-12 page-info">
                    ...
                    <div>
                        ...
                        <span>{{trans('content.editPriceOffer')}}</span>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="form-data">
                    ...
                    @endif
                    <form class="" name="edit-price-offer" action="{{route('prices_offers.update', $price_offer->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="branch" class="branch form-select">
                                        ...
                                        @foreach($branches as $branch)
                                            <option value="{{$branch->id}}" @if($branch->id == $price_offer->branch_id) selected @endif>{{$branch->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('branch')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="store" class="store form-select">
                                        ...
                                        @foreach($stores as $store)
                                            <option value="{{$store->id}}" @if($store->id == $price_offer->store_id) selected @endif>{{$store->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('store')
                                    <small class="text-danger">{{$message}}</small>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="item[]" class="item form-select">
                                        <optgroup label="{{trans('content.enterItemName')}}"></optgroup>
                                        <option value="" disabled selected class="disabled-option"></option>
                                        @foreach($items as $item)
                                            <option value="{{$item->id}}" @if(in_array($item->id, $offer_items)) selected @endif>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('item')
                                    <small class="text-danger">{{$message}}</small>
        ...
        $(".store").change(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    {
                        $(".item").append("<option multiple='multiple' value=" + response.data[i]['item_id'] + ">" + response.data[i]['item_name'] + "</option>");
                    }
                },
                error: function (resErr) {
                }
