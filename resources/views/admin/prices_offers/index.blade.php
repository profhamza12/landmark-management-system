            ...
            <div class="row">
                <div class="col-sm-12 page-info">
                    ...
                    <div>
                        ...
                        <span>{{trans('content.prices_offers')}}</span>
                    </div>
                </div>
                <div class="col-sm-12 table-data">
                    ...
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                <th>{{trans('content.priceOfferId')}}</th>
                        ...
                        <tbody>
                            @isset($prices_offers)
                                @foreach($prices_offers as $price_offer)
                                    <tr class="tr_{{$price_offer->id}}">
                                        <td>{{$price_offer->id}}</td>
                                        <td>{{$price_offer->created_at}}</td>
                                        <td>
                                            <a href="{{route('prices_offers.show', $price_offer->id)}}" class="btn btn-outline-info">{{trans('content.showPriceOffer')}}</a>
                                            <a href="{{route('prices_offers.edit', $price_offer->id)}}" class="btn btn-outline-primary">{{trans('content.edit')}}</a>
                                            <a href="" data-id="{{$price_offer->id}}" class="entry-delete btn btn-outline-danger">{{trans('content.delete')}}</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @endisset
    ...
    <script>
        $(function () {
            $(".entry-delete").click(function (e) {
                ...
                if (window.confirm("{{trans('content.priceofferdeletepromptmsg')}}"))
                {
                    let entryid = $(this).data('id');
                    let url = "{{route('prices_offers.destroy', ":id")}}";
