            ...
            <div class="row">
                <div class="col-sm-12 page-info">
                    ...
                    <div>
                        ...
                        <span>{{trans('content.sales_invoices')}}</span>
                    </div>
                </div>
                            ...
                            @isset($invoices)
                                @foreach($invoices as $invoice)
                                    <tr class="tr_{{$invoice->id}}">
                                        ...
                                        <td>
                                            <a href="{{route('sales_invoices.show', $invoice->id)}}" class="btn btn-outline-info">{{trans('content.showInvoice')}}</a>
                                            @if ($invoice->relayed == 0)
                                                <a href="{{route('sales_invoices.edit', $invoice->id)}}" class="edit btn btn-outline-primary">{{trans('content.edit')}}</a>
                ...
                buttons:  [
                    {
                        ...
                        "filename": "{{trans('content.sales_invoices')}}",
                        autoPrint: true,
                        exportOptions: {
                            columns: [ 0, 1 ]
                        },
        ...
        $(function () {
            $(".entry-delete").click(function (e) {
                ...
                if (window.confirm("{{trans('content.invoicedeletepromptmsg')}}"))
                {
                    ...
                    let url = "{{route('sales_invoices.destroy', ":id")}}";
            ...
            $(".invoice-relay").click(function (e) {
                ...
                {
                    let invoiceId = $(this).data('id');
                    let url = "{{route('sales_invoices.relay', ":id")}}";
                    url = url.replace(":id", invoiceId);
                    $.ajax({
                        method: 'POST',
                        url: url,
