        ...
        <div class="wraper-content">
            <div class="row">
                <div class="col-sm-12 page-info">
                    <h2>{{trans('content.show_inventory_entry')}}</h2>
                    <div>
                        <a href="">{{trans('content.home')}}</a> -
                        <span>{{trans('content.show_inventory_entry')}}</span>
                    </div>
                </div>
                            ...
                            @isset($store_quantities)
                                @foreach($store_quantities as $_item)
                                    <tr>
                                        ...
                                        <td>{{$inventory_entry->Branch->name}}</td>
                                        <td>{{$inventory_entry->Store->name}}</td>
            ...
            $customiseData =
                "<div class='payable-custom-data'>\
                    <div class='payable-branch'>\
                        <span>{{trans('content.branch') . " / " . $inventory_entry->Branch->name}}</span>\
                    </div>\
                    <div class='payable-date'>\
                        <span>{{trans('content.date') . " / " . $inventory_entry->created_at}}</span>\
                    </div>\
                    <div class='payable-code'>\
                        <span>{{trans('content.payable_code') . " / " . $inventory_entry->id}}</span>\
                    </div>\
                </div>";
            let language_ar = {
                ...
                buttons:  [
                    {
                        "charset": "utf-8",
                        "extend": "print",
                        "title": "{{trans('content.inventory_entry')}}",
                        "text": "{{trans('content.print')}}",
                        "filename": "{{trans('content.inventory_entry')}}",
                        autoPrint: true,
                        exportOptions: {
                    ...
                    },
                    {
                        ...
                        "extend": "csv",
                        "title": "{{trans('content.payable_entry')}}",
                        "messageTop": "{{trans('content.date') . " / " . ($inventory_entry->created_at)}}",
