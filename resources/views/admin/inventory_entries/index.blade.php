            ...
            <div class="row">
                <div class="col-sm-12 page-info">
                    ...
                    <div>
                        ...
                        <span>{{trans('content.inventory_entries')}}</span>
                    </div>
                </div>
                <div class="col-sm-12 table-data">
                    ...
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                <th>{{trans('content.InventoryEntryId')}}</th>
                                <th>{{trans('content.date')}}</th>
                                <th>{{trans('content.control')}}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @isset($inventory_entries)
                                @foreach($inventory_entries as $entry)
                                    <tr class="tr_{{$entry->id}}">
                                        <td>{{$entry->id}}</td>
                                        <td>{{$entry->created_at}}</td>
                                        <td>
                                            <a href="{{route('inventory_entries.show', $entry->id)}}" class="btn btn-outline-info">{{trans('content.showInventoryEntry')}}</a>
                                            <a href="" data-id="{{$entry->id}}" class="entry-delete btn btn-outline-danger">{{trans('content.delete')}}</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @endisset
                ...
                buttons:  [
                    {
                        ...
                        "filename": "{{trans('content.inventory_entries')}}",
                        autoPrint: true,
                        exportOptions: {
                            columns: [ 0, 1 ]
                        },
        ...
        $(function () {
            $(".entry-delete").click(function (e) {
                ...
                if (window.confirm("{{trans('content.entrydeletepromptmsg')}}"))
                {
                    ...
                    let url = "{{route('inventory_entries.destroy', ":id")}}";
