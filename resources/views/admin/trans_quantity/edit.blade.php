            ...
            <div class="row">
                <div class="col-sm-12 page-info">
                    ...
                    <div>
                        ...
                        <span>{{trans('content.editTransQuantity')}}</span>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="form-data">
                    ...
                    @endif
                    <form class="" name="edit-trans-quantity-op" action="{{route('trans_quantity.update', $store_trans_operation->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="branch" class="branch form-select">
                                        ...
                                        @foreach($branches as $branch)
                                            <option value="{{$branch->id}}" @if($branch->id == $store_trans_operation->branch_id) selected @endif>{{$branch->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('branch')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="src_store" class="store form-select">
                                        ...
                                        @foreach($stores as $store)
                                            <option value="{{$store->id}}" @if($store->id == $store_trans_operation->src_store) selected @endif>{{$store->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('src_store')
                                    <small class="text-danger">{{$message}}</small>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="dest_store" class="store form-select">
                                        ...
                                        <option disabled selected class="disabled-option"></option>
                                        @foreach($stores as $store)
                                            <option value="{{$store->id}}" @if($store->id == $store_trans_operation->dest_store) selected @endif>{{$store->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('dest_store')
                                    <small class="text-danger">{{$message}}</small>
                        ...
                        <div class="row">
                            <div class="store_quantity">
                                ...
                                @foreach($operation_details as $index => $_item)
                                    <div class="sec">
                                        ...
                                        <div class="row">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select name="store_quantity[{{$index}}][unit]" class="form-select">
                                                        ...
                                                        @foreach($units as $unit)
                                                            <option value="{{$unit->id}}" @if($_item->unit_id == $unit->id) selected @endif>{{$unit->name}}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <input type="number" name="store_quantity[{{$index}}][quantity]" value="{{$_item->quantity}}" class="form-control" placeholder="{{trans('content.enterItemQuantity')}}" min="0">
