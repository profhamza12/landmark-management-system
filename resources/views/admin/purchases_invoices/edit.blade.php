            ...
            <div class="row">
                <div class="col-sm-12 page-info">
                    ...
                    <div>
                        ...
                        <a href="">{{trans('content.purchases_invoices')}}</a> -
                        <span>{{trans('content.editPurchaseInvoice')}}</span>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="form-data">
                    ...
                    @endif
                    <form class="" name="edit-purchases-invoice" action="{{route('purchases_invoices.update', $invoice->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="branch" class="branch form-select">
                                        ...
                                        @foreach($branches as $branch)
                                            <option value="{{$branch->id}}" @if($invoice->branch_id == $branch->id) selected @endif>{{$branch->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('branch')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="store" class="store form-select">
                                        ...
                                        @foreach($stores as $store)
                                            <option value="{{$store->id}}" @if($store->id == $invoice->store_id) selected @endif>{{$store->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('store')
                                    <small class="text-danger">{{$message}}</small>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="vendor" class="form-select">
                                        ...
                                        @foreach($vendors as $vendor)
                                            <option value="{{$vendor->id}}" @if($vendor->id == $invoice->vendor_id) selected @endif>{{$vendor->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('vendor')
                                    <small class="text-danger">{{$message}}</small>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="invoice_type" class="invoice_type form-select">
                                        ...
                                        @foreach($invoice_types as $invoice_type)
                                            <option value="{{$invoice_type->id}}" @if($invoice_type->id == $invoice->invoice_type) selected @endif data-name="{{$invoice_type->name}}">{{$invoice_type->display_name}}</option>
                                        @endforeach
                                    </select>
                                    @error('invoice_type')
                                    <small class="text-danger">{{$message}}</small>
                        ...
                        <div class="row">
                            <div class="store_quantity">
                                <div class="add-sec">
                                    ...
                                    <h2 class="d-inline-block">{{trans('content.purchases_invoice_quantities')}}</h2>
                                </div>
                                @foreach($invoice_items as $index => $invoice_item)
                                    <div class="sec">
                                        <h6>{{trans('content.purchases_invoice_quantity')}}</h6>
                                        <div class="row">
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label>{{trans('content.item')}}</label>
                                                    <select name="store_quantity[{{$index}}][item]" class="item form-select">
                                                        <optgroup label="{{trans('content.enterItem')}}"></optgroup>
                                                        <option selected disabled></option>
                                                        @foreach($items as $item)
                                                            <option value="{{$item->id}}" @if($item->id == $invoice_item->item_id) selected @endif>{{$item->name}}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <select name="store_quantity[{{$index}}][unit]" class="unit form-select">
                                                        <optgroup label="{{trans('content.enterUnit')}}"></optgroup>
                                                        <option selected disabled></option>
                                                        @foreach($units as $unit)
                                                            <option value="{{$unit->id}}" @if($unit->id == $small_unit->id) selected @endif>{{$unit->name}}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <input type="number" name="store_quantity[{{$index}}][quantity]" class="quantity form-control" placeholder="{{trans('content.enterItemQuantity')}}" min="0" value="{{$invoice_item->quantity}}">
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label>{{trans('content.itemPurchaseBrice')}}</label>
                                                    <input type="number" name="purchasing_price" class="purchasing_price form-control" readonly value='0'>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" name="total_amount" class="total_amount form-control" readonly value="{{@$invoice->total_amount}}">
                                    @error('total_amount')
                                    <small class="text-danger">{{$message}}</small>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" name="discount" class="discount form-control" value="{{@$invoice->discount}}">
                                    @error('discount')
                                    <small class="text-danger">{{$message}}</small>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" name="paid_amount" class="paid_amount form-control" value="{{@$invoice->paid_amount}}">
                                    @error('paid_amount')
                                    <small class="text-danger">{{$message}}</small>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" name="remaining_amount" class="remaining_amount form-control" readonly value="{{@$invoice->remaining_amount}}">
...
@section('ajaxscript')
    <script>
        $(function () {
            $(".purchasing_price").val($(".total_amount").val());
            $(".branch").change(function () {
                $(".store option").not(".disabled-option").remove();
                $branch_id = $(this).val();
                $url = '{{route('purchases_invoices.get_branch_stores', ':id')}}';
            ...
            $(".store").change(function () {
                ...
                $store_id = $(this).val();
                $url = '{{route('purchases_invoices.get_store_items', ':id')}}';
            ...
            $("body").on("blur", ".quantity", function () {
                ...
                $purchasing_price = $(this).parents(".sec").find(".purchasing_price");
                $total_amount = 0;
                if ($quantity !== "" && $unit_id !== "" && $item_id !== "")
                {
                    $.ajax({
                        method: 'POST',
                        url: "{{route('purchases_invoices.get_unit_detail')}}",
                        data: {_token: "{{csrf_token()}}", unit_id: $unit_id, item_id: $item_id, quantity: $quantity},
                        success: function (response) {
                            $purchasing_price.val(response.purchasing_price);
                            $(".sec .purchasing_price").each(function ($index) {
